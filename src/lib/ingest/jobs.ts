  ...
  return withRun("fund-catalog", { provider: provider.name }, async () => {
    ...
    for (const batch of chunk(entries)) {
      const rows = batch.map((entry) => ({
        ...
        // No default stands in for a künye field the source omits: the row
        // stays empty and the app reads it as "Bilinmiyor".
        withholdingTax: entry.withholdingTax ?? null,
        risk: entry.risk ?? null,
        buyValueDays: entry.buyValueDays ?? null,
        sellValueDays: entry.sellValueDays ?? null,
        fundType: entry.fundType ?? "YAT",
        onTefas: entry.onTefas ?? null,
      ...
      const result = await db
        ...
        .onConflictDoUpdate({
          ...
          set: {
            ...
            withholdingTax: sql`coalesce(excluded.withholding_tax, ${funds.withholdingTax})`,
            risk: sql`coalesce(excluded.risk, ${funds.risk})`,
            buyValueDays: sql`coalesce(excluded.buy_value_days, ${funds.buyValueDays})`,
            sellValueDays: sql`coalesce(excluded.sell_value_days, ${funds.sellValueDays})`,
            fundType: sql`excluded.fund_type`,
            onTefas: sql`coalesce(excluded.on_tefas, ${funds.onTefas})`,
